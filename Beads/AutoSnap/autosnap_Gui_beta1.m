function varargout = autosnap_Gui_beta1(varargin)
% AUTOSNAP_GUI_BETA1 MATLAB code for autosnap_Gui_beta1.fig
%      AUTOSNAP_GUI_BETA1, by itself, creates a new AUTOSNAP_GUI_BETA1 or raises the existing
%      singleton*.
%
%      H = AUTOSNAP_GUI_BETA1 returns the handle to a new AUTOSNAP_GUI_BETA1 or the handle to
%      the existing singleton*.
%
%      AUTOSNAP_GUI_BETA1('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in AUTOSNAP_GUI_BETA1.M with the given input arguments.
%
%      AUTOSNAP_GUI_BETA1('Property','Value',...) creates a new AUTOSNAP_GUI_BETA1 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before autosnap_Gui_beta1_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to autosnap_Gui_beta1_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help autosnap_Gui_beta1

% Last Modified by GUIDE v2.5 23-Jul-2014 15:15:31

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @autosnap_Gui_beta1_OpeningFcn, ...
                   'gui_OutputFcn',  @autosnap_Gui_beta1_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before autosnap_Gui_beta1 is made visible.
function autosnap_Gui_beta1_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to autosnap_Gui_beta1 (see VARARGIN)

% Choose default command line output for autosnap_Gui_beta1
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes autosnap_Gui_beta1 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = autosnap_Gui_beta1_OutputFcn(hObject, eventdata, handles) 
...
if handles.pads==0
    ...
    autosnap_beta_error_send(handles.pads,handles.number_of_pos, handles.posname, handles.cstring, get(handles.output_folder, 'String'), handels.posname_title)
end
if get(handles.auto_move,'Value')==1
    ...
    handles.foldername=[handles.move_files,'\',handles.folder];
    if exist(handles.foldername,'file')==0
        mkdir(handles.foldername);
    end
    cd(handles.foldername);
    D=dir;
    handles.foldername_take=[handles_foldername,'\Take',num2str(length(D)+1)]);
    mkdir(handles.foldername_take);
    cd(get(handles.path,'String'));
    D=dir('*tif');
    for i=1:length(D)
        copy([get(handles.path,'String'),'\',D(i).name],handles.foldername_take);
    end
    cd(get(handles.path,'String'));
    D=dir('*stg');
    for i=1:length(D)
    ...
    copy([get(handles.path,'String'),'\',handles.output_folder],handles.foldername_take);
end
