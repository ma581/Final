function redseg_better_del_no_cells_auto_no_schmutz2(fname, varargin)
...
%Creating Folder for deleted files
currentfolder=pwd;
if exist([currentfolder,'\Deleted Images'],'file')==0;
...
% Asigning value to Timestr, Gainstr, Exptime if they are empty
    ...
    for i = 1:Nimgs,
        ...
        if exist(pname),
            imp = imread(pname);
            im_std=std2(imp);
            %if im_std>15&&im_std<200; %deletes images which have a smaller variance than 15
                psize = size(imp);
    ...
    %             [dstr,tstr,dn] = imdatetime(pname); %CHECK:  Do we really need imdatetime? [Jon]
                ...
                for l= 1:size(ph3,3),
                    ph3 = ph3;
                    ph3(:,:,l)= medfilt2(ph3(:,:,l),[3 3]); %operates on image
                    x= double(ph3(:,:,l));
                    ...
                    ph3(:,:,l)= uint16(10000*rescaled);%rescaling pixels
                end;
    %------------------------------------------------------------------------------
    ...
    %--------------------------------------------------------------------------------
                ...
                [T,cont]=progthreshfluor(ph3); %Main segmentation function
                if cont==1
                    ...
                    if ~isempty(s),
                        ...
                        if naturalback,
                            for k = 1:length(s),
                                lowbox = floor(max([ones(1,2); s(k).Centroid - 64]));
                                highbox = floor(min([size(imp'); s(k).Centroid + 64]));
                                for kk = 1:length(imf),
                    ...
                    disp(['skipping too much schmutz',pname]);
                    fileID = fopen('names.txt','w');
                    del_l=length(del_names);
                    del_names{del_l+1}=D(i).name;
...
%--------------------------------------------------------------------------
function [progfluor,continue_ana] = progthreshfluor(imt)
p = []; %Initialize p
...
if max2(L3)>200
    disp('Too much Schmutz. Stopping redseg for this image.');
    continue_ana=0;
    progfluor = 0;
    return;
end
