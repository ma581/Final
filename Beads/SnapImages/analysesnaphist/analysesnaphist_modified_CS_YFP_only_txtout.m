 function analysesnaphist_modified(varargin)
global savedata;
...
if savedata ==1 
    histo=[binpositionsc; normcountsc];
    filename=[input,'_histo','.txt'];
...
if savedata ==1 
    gamma=[xc; histareac*yc];
    filename=[input,'_gamma','.txt'];
    fileID=fopen(filename,'w');
    fprintf(fileID,'%6.8f %6.8f\r\n', gamma);
    fclose(fileID);
end
