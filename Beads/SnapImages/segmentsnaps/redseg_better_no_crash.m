...
bw2a = ismember(L, goodones); %filters out difference between two vectors
L2a = bwlabel(bw2a);

%removing hughe cells
r1a = regionprops(L2a,'Area');
huge = find([r.Area]>50000);
disp(['Removing small (crud) cells(', num2str(length(flittle)),').']);
goodones2a = setdiff(1:max(max(L2a)), huge);
bw2 = ismember(L2a, goodones2a); %filters out difference between two vectors
L2 = bwlabel(bw2);
